...
efficient_file = os.path.join(
    SETTINGS_JSON['MODEL_PREDICT_DIR'], 'efficientnet.h5')
efficient_input_size = PARAMS_JSON['EFFICIENTNET_INPUT_SIZE']

#Load Classification Model
efficient = get_eff_model(eff, 0, (efficient_input_size, efficient_input_size, 3),
                          PARAMS_JSON['CLASS_NUM'], lr=1e-3, dropout=0.2, weights=efficient_file)
...
for index, test_image_name in enumerate(test_image_names):
    sys.stdout.write(
        "Predicting: {}/{} ...\r".format(index+1, len(test_image_names)))
    sys.stdout.flush()

    img = io.imread(os.path.join(path_to_test_images,
                    test_image_name), as_gray=True)
    img = img/255.
...
print('=======================================================================')
print('Consolidation Accuracy: {}'.format(Accuracy(true[:, 0].reshape(
    len(true), 1), prediction[:, 0].reshape(len(prediction), 1))))
print('Consolidation Precision: {}'.format(Precision(true[:, 0].reshape(
    len(true), 1), prediction[:, 0].reshape(len(prediction), 1))))
print('Consolidation Recall: {}'.format(Recall(true[:, 0].reshape(
    len(true), 1), prediction[:, 0].reshape(len(prediction), 1))))

print('Pneumothorax Accuracy: {}'.format(Accuracy(true[:, 1].reshape(
    len(true), 1), prediction[:, 1].reshape(len(prediction), 1))))
print('Pneumothorax Precision: {}'.format(Precision(true[:, 1].reshape(
    len(true), 1), prediction[:, 1].reshape(len(prediction), 1))))
print('Pneumothorax Recall: {}'.format(Recall(true[:, 1].reshape(
    len(true), 1), prediction[:, 1].reshape(len(prediction), 1))))

print('Edema Accuracy: {}'.format(Accuracy(true[:, 2].reshape(
    len(true), 1), prediction[:, 2].reshape(len(prediction), 1))))
print('Edema Precision: {}'.format(Precision(true[:, 2].reshape(
    len(true), 1), prediction[:, 2].reshape(len(prediction), 1))))
print('Edema Recall: {}'.format(Recall(true[:, 2].reshape(
    len(true), 1), prediction[:, 2].reshape(len(prediction), 1))))

print('Effusion Accuracy: {}'.format(Accuracy(true[:, 3].reshape(
    len(true), 1), prediction[:, 3].reshape(len(prediction), 1))))
print('Effusion Precision: {}'.format(Precision(true[:, 3].reshape(
    len(true), 1), prediction[:, 3].reshape(len(prediction), 1))))
print('Effusion Recall: {}'.format(Recall(true[:, 3].reshape(
    len(true), 1), prediction[:, 3].reshape(len(prediction), 1))))

print('Pneumonia Accuracy: {}'.format(Accuracy(true[:, 4].reshape(
    len(true), 1), prediction[:, 4].reshape(len(prediction), 1))))
print('Pneumonia Precision: {}'.format(Precision(true[:, 4].reshape(
    len(true), 1), prediction[:, 4].reshape(len(prediction), 1))))
print('Pneumonia Recall: {}'.format(Recall(true[:, 4].reshape(
    len(true), 1), prediction[:, 4].reshape(len(prediction), 1))))

print('Cardiomegaly Accuracy: {}'.format(Accuracy(true[:, 5].reshape(
    len(true), 1), prediction[:, 5].reshape(len(prediction), 1))))
print('Cardiomegaly Precision: {}'.format(Precision(true[:, 5].reshape(
    len(true), 1), prediction[:, 5].reshape(len(prediction), 1))))
print('Cardiomegaly Recall: {}'.format(Recall(true[:, 5].reshape(
    len(true), 1), prediction[:, 5].reshape(len(prediction), 1))))
print('=======================================================================')
