...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        image_of_array, label_of_onehot = self.__data_generation(
            list_IDs_temp, labels_temp)
    ...
    def __data_generation(self, list_IDs_temp, labels_temp):
        ...
        """
        image_of_array = np.empty(
            (self.batch_size, *self.dim, self.n_channels), dtype=np.uint8)
        label_of_onehot = np.empty(
            (self.batch_size, self.n_classees), dtype=np.uint8)

        #load image
        for i, ID in enumerate(list_IDs_temp):
            image_of_array[i, ] = np.load(
                os.path.join(self.data_dir, ID)+'.npy')
        ...
        if self.preprocess_input is not None and self.model_name is not None:
            image_of_array = self.preprocess_input(
                image_of_array, self.model_name)
        ...
        if self.to_categori is not None:
            ...
            label_of_onehot = np.asarray(
                self.to_categori(labels, self.n_classees))
        else:
