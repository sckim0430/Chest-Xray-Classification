...
df = df[['Path', 'Consolidation', 'Pneumothorax', 'Edema',
         'Pleural Effusion', 'Pneumonia', 'Cardiomegaly']]
df.columns = ['Path', 'Consolidation', 'Pneumothorax',
...
for index in range(len(df)):
    ...
    if not bool_df.loc[index].any():  # delete index for no disease info in csv file
        ...
        dst_dir = os.path.join(
            SETTINGS_JSON['DATASET_DIR'], 'merged_images', 'Chexpert_{}.jpg'.format(count))
